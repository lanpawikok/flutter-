...
class AuthService {
  static Future<bool> login(String username, String password) async {
    ...
    try {
      final response = await http.post(
        ...
        headers: {
          'Content-Type':
              'application/json', // Wajib memberi tahu server bahwa ini JSON
        },
  ...
  /// Fungsi untuk menghapus token dan data session saat user logout
  static Future<void> logout() async {
    final prefs = await SharedPreferences.getInstance();

    // Menghapus data spesifik agar session bersih
